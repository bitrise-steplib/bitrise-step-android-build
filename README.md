...
| `BITRISE_ANDROID_ARTIFACT_MAP_PATH` | Path of a JSON file, written next to the exported artifacts in the deploy directory, that records which exported APK/AAB files and which R8/ProGuard mapping file belong to the same build variant. Use it when a single build produces multiple variants and a later step (for example Google Play deploy) needs the matching mapping file for each app artifact.  File references inside the document are file names relative to the JSON file's own directory. Example:  ``` {   "version": 1,   "variants": {     "demoRelease": {       "module": "app",       "mapping": "mapping.txt",       "aab": [],       "apk": ["app-demo-release.apk"]     },     "paidRelease": {       "module": "app",       "mapping": "mapping-20260805121530.txt",       "aab": [],       "apk": ["app-paid-release.apk"]     }   },   "unmatched": { "apk": [], "aab": [], "mapping": [] } } ```  Only the selected **Build type**'s list is populated (`apk` or `aab`). `unmatched` lists exported files whose variant could not be derived from their build output path. When two modules produce the same variant name, the keys are disambiguated as `module/variant`. |
</details>
