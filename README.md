...
| --- | --- |
| `BITRISE_APK_PATH` | This output will include the path of the generated APK after filtering based on the filter inputs. If the build generates more than one APK which fulfills the filter inputs, this output will contain the last one's path.  When building multiple variants, prefer `$BITRISE_ANDROID_ARTIFACT_MAP_PATH`: it tells you which APK belongs to which variant instead of only the last one. |
| `BITRISE_APK_PATH_LIST` | This output will include the paths of the generated APKs after filtering based on the filter inputs. The paths are separated with `\|` character, for example, `app-armeabi-v7a-debug.apk\|app-mips-debug.apk\|app-x86-debug.apk`  When building multiple variants, prefer `$BITRISE_ANDROID_ARTIFACT_MAP_PATH`: it groups the APKs by variant and pairs each variant with its mapping file. |
| `BITRISE_AAB_PATH` | This output will include the path of the generated AAB after filtering based on the filter inputs. If the build generates more than one AAB which fulfills the filter inputs, this output will contain the last one's path.  When building multiple variants, prefer `$BITRISE_ANDROID_ARTIFACT_MAP_PATH`: it tells you which AAB belongs to which variant instead of only the last one. |
| `BITRISE_AAB_PATH_LIST` | This output will include the paths of the generated AABs after filtering based on the filter inputs. The paths are separated with `\|` character, for example, `app--debug.aab\|app-mips-debug.aab`  When building multiple variants, prefer `$BITRISE_ANDROID_ARTIFACT_MAP_PATH`: it groups the AABs by variant and pairs each variant with its mapping file. |
...
| `BITRISE_ANDROID_ARTIFACT_MAP_PATH` | Path of a JSON file, written next to the exported artifacts in the deploy directory, that records which exported APK/AAB files and which R8/ProGuard mapping file belong to the same build variant. Use it when a single build produces multiple variants and a later step (for example Google Play deploy) needs the matching mapping file for each app artifact.  File references inside the document are file names relative to the JSON file's own directory. Entries are keyed by Gradle module, then by variant — build variants are a per-module concept, and two modules can declare the same variant name. Example:  ``` {   "version": 1,   "modules": {     "app": {       "demoRelease": {         "mapping": "mapping.txt",         "aab": [],         "apk": ["app-demo-release.apk"]       },       "paidRelease": {         "mapping": "mapping-20260805121530.txt",         "aab": [],         "apk": ["app-paid-release.apk"]       }     }   },   "unmatched": { "apk": [], "aab": [], "mapping": [] } } ```  Only the selected **Build type**'s list is populated (`apk` or `aab`). `unmatched` lists exported files whose module/variant could not be derived from their build output path. |
</details>
